...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  3 20:07:34 2022
...
def getReviewer(html, driver, number):
    ...
    name  = driver.find_elements_by_class_name("crux-page-title")[0].text.strip()
    print(name)
    dic["name"] = name
    print("First part")
   ...
   #              value = "poor"
    ...
    #print(dic)
    field_file = open('fields1.txt', 'r')
    FLines = field_file.readlines()
    fields = [];
    for l in FLines:
        fields.append(l.strip())
    with open("result0204recall_part3.csv", 'a+') as csvfile:
...
if __name__ == "__main__":
    ...
    for i in range(2918, len(htmlarray)):
    #for i in range(2085, 2086):
            if len(htmlarray[i]) != 0:
                ...
                cur = {}
                cur["no_html"] = i
                with open("record_0204.csv", 'a+') as csvfile:
